...
" to "+a.format("h:mm a");d=JSON.stringify({status_text:d,status_emoji:f,status_expiration:a.utcOffset(-10,!0).unix()});console.log(d);slack.users.profile.set({token:b,profile:d});rememberToGoRedTime=a.unix()+30;text="shift over";slack.reminders.add({token:b,text:text,rememberToGoRedTime:rememberToGoRedTime});console.log("setting reminder to go red for "+rememberToGoRedTime);c.status(200);c.send("\ud83e\udd18")}else b()});
